...
import { I18nManager, ImageBackground, ScrollView, View } from "react-native"
...
import { signupAddPersonalDataInSystem } from "../../../middleware/authentication/register"

const CompleteAddress: React.FC<{}> = (params: any) => {
    ...
    const onComplete = () => {
      ...
      // }
        ...
        const data2 = {
          ...
          branch_id: endpoints?.branch_code,
          centerId: endpoints?.branch_code,
          firstName: paramsData?.firstName,
          lastName: "",
          mobileNumber: paramsData?.mobileNumber,
          email: paramsData?.email,
          dob: formatDate(paramsData?.dob),
          otpVerified: 1,
          password: paramsData?.password,
          confirmPassword: paramsData?.password,
          gender: paramsData?.gender?.id,
          height: paramsData?.height?.id,
          weight: paramsData?.weight?.id,
          deliveryTime: selectDeliveryTime?.id,
          addressDetails: {
            area: selectArea?.id,
            deliveryTime: selectDeliveryTime?.id,
            block,
        ...
        dispatch(signupAddPersonalDataInSystem(data2))
      }
    }
    return (
